...
def build_webhook_payload(job: dict[str, Any], config: AppConfig, stats: dict[str, Any] | None = None) -> str:
    if getattr(config, "use_henrik_stats", False):
        if stats and stats.get("available"):
            rank = stats.get("rank") or "Unknown"
            level = stats.get("account_level")
            if level is None:
                level = stats.get("level")
            return f"Rank: **{_discord_bold_value(rank)}**\nLevel: **{_discord_bold_value(level if level is not None else 'Unknown')}**"
        return "Valorant stats unavailable."
    return ""
...
def upload_processed_job(
    ...
    if getattr(config, "use_henrik_stats", False):
        logger.info(
            "Discord upload stats config for job %s: enabled=True name_present=%s tag_present=%s region=%s api_key_present=%s.",
            job.get("id"),
        ...
        stats_result = fetch_valorant_stats(config)
    ...
    content = build_webhook_payload(job, config, stats)
    logger.info("Discord upload message mode for job %s: %s.", job.get("id"), message_mode)
